const HELP_TAB_PANE = `
    <div class="tab-pane m-2" id="helpTabContent" role="tabpanel">
        <div class="mt-3">
            <div class="container mt-4">
                <div class="row">
                    <div class="col-md-12">
                        <span data-i18n="optionsHelpText"></span>&nbsp;&hellip;
                    </div>
                </div>
                <div class="row align-items-center m-4">
                    <div class="col-auto">
                        <div class="btn-group">
                            <a href="https://www.achimonline.de/mozext/" class="btn btn-link">
                                <img id="helpImageMozillaDino" class="img-fluid"><br>
                                <span class="nobr">www.achimonline.de/mozext/</span>
                            </a>
                        </div>
                    </div>
                    <div class="col-auto">
                        &hellip;&nbsp;<span data-i18n="optionsHelpOr"></span>&nbsp;&hellip;
                    </div>
                    <div class="col-auto">
                        <div class="btn-group">
                            <a href="https://github.com/4ch1m/mozext/tree/master/signatureswitch-me" class="btn btn-link">
                                <img id="helpImageGithubOctocat" class="img-fluid"><br>
                                <span class="nobr">github.com/4ch1m/mozext</span>
                            </a>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <hr class="w-100">
                </div>
                <div class="row mt-2">
                    <div class="col-md-1 text-center">
                        <svg class="bi" width="2em" height="2em" fill="#ff0000">
                            <use xlink:href="/_images/bootstrap-icons.svg#heart-fill"/>
                        </svg>
                    </div>
                    <div class="col-md-11">
                        <span data-i18n="optionsHelpSupport1"></span>&nbsp;&hellip;<br>
                        <a href="https://www.achimonline.de/mozext/contact.php">Achim's Mozilla Extensions <small><small>(https://www.achimonline.de/mozext/contact.php)</small></small></a><br>
                        &hellip;&nbsp;<span data-i18n="optionsHelpSupport2"></span>
                    </div>
                </div>
            </div>
        </div>
    </div>
`;
